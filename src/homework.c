...
{
	double phi[3];
	double den = (X[0] * Y[1] - X[1] * Y[0] - X[0] * Y[2] + X[2] * Y[0] + X[1] * Y[2] - X[2] * Y[1]);
	double xsi = -(X[0] * Y[2] - X[2] * Y[0] - X[0] * x[1] + Y[0] * x[0] + X[2] * x[1] - Y[2] * x[0]) / den;
	double eta = (X[0] * Y[1] - X[1] * Y[0] - X[0] * x[1] + Y[0] * x[0] + X[1] * x[1] - Y[1] * x[0]) / den;

	femElementType tri = FEM_TRIANGLE;

	femDiscretePhi2(theProblemX->space, xsi, eta, phi);

	// printf("xsi %f eta %f  phi0 %f phi1 %f phi2 %f\n", xsi, eta, phi[0], phi[1], phi[2]);
