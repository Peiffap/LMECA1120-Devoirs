...
void femDiffusionFree(femDiffusionProblem *theProblem)
{
	...
	femIterativeSolverFree(theProblem->solver);
...
void femDiffusionCompute(femDiffusionProblem *theProblem)
{
	...
	femIterativeSolver *theSolver = theProblem->solver;
	...
	for (iEdge = 0; iEdge < theEdges->nEdge; iEdge++) {
		if (theEdges->edges[iEdge].elem[1] < 0) {
			femIterativeSolverConstrain(theSolver, number[theEdges->edges[iEdge].node[0]], 0.0);
			femIterativeSolverConstrain(theSolver, number[theEdges->edges[iEdge].node[1]], 0.0);
		}
	}
